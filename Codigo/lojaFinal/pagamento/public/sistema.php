...
if (isset($_SESSION['checkout_session_id'])) {
    ...
    try {
        ...
        // Outras verificações...
        $usuarioId = isset($_SESSION['id_cliente']) ? $_SESSION['id_cliente'] : null;
        ...
        if ($session && $session->payment_status == 'paid') {
            ...
            $data = date("Y-m-d H:i:s", $createdTimestamp);
            $productIds = array_map(function ($product) {
                return $product['id'];
            }, $productsInCart);
            $idProdutos = implode(',', $productIds);

            $stmt = $conn->prepare("INSERT INTO pagamentos (valor, moeda, status, data_pagamento, usuario_id, idProdutos) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("dsssis", $valor, $moeda, $status, $data, $usuarioId, $idProdutos);
            $stmt->execute();
